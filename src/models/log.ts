...
export class LogModel {
   ...
   * This completely eliminates the 320k+ row full-table scans previously caused by `SELECT MAX(hour_timestamp)`
   * on secondary-index-less rollup tables without requiring any write-amplifying secondary indexes.
   *
  ...
  async getTopAllowed(
    ...
    if (accessPointId) {
      ...
      const { results } = await this.db.prepare(queryStr).bind(profileId, since, until, accessPointId).all<{ domain: string; count: number }>();
      return results;
    }
    ...
    if (cutoff <= since) {
      ...
      const { results } = await this.db.prepare(queryStr).bind(profileId, since, until).all<{ domain: string; count: number }>();
      return results;
    }
    ...
    if (cutoff > until) {
      ...
      const queryStr = `
        ...
        LIMIT 10
      `;
      const { results } = await this.db.prepare(queryStr).bind(profileId, sinceHour, until).all<{ domain: string; count: number }>();
      return results;
    }
